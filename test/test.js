...
const Polls = require('../app/models/polls');
...
describe('Unit Testing', function() {
  describe('colors.js', function() {
    ...
    it('colorLuminance should accept a hex color of length 6 and return a '
      + 'color of length 6',
    function(done) {
      assert.equal(
          colors.colorLuminance('#f6e550', colors.HIGHLIGHT_LUMINANCE),
          '#ffff60');
      done();
    });

    it('colorLuminance should accept a hex color of length 3 and return a '
      + 'color of length 6',
    function(done) {
      assert.equal(
          colors.colorLuminance('#fe5', colors.HIGHLIGHT_LUMINANCE),
          '#ffff66');
      done();
    });
  });
  ...
  describe('Models', function() {
    describe('Users', function() {
      it('should create a new User', function(done) {
        const user = {username: 'user', password: 'unhashed-password-0'};
        Users.create(user, function(err, newUser) {
          assert.isNotOk(err, 'Error while creating new user');
          assert.equal(
              user.username, newUser.username,
              'The new user doesn\'t have the correct username');
          assert.equal(
              user.password, newUser.password,
              'The new user doesn\'t have the correct password');
          userID = newUser._id;
          done();
        });
      });
    ...
    describe('Polls', function() {
      it('should create a new Poll', function(done) {
        poll = {
          ...
          options: [
            {answer: 'First Option', votes: 0, color: '#00c64a'},
            {answer: 'Second Option', votes: 0, color: '#00b67f'},
          ],
        ...
        Polls.create(poll, function(err, newPoll) {
          ...
          assert.equal(
              ...
              'The new poll doesn\'t have the correct question');
          for (let i = 0; i < poll.options.length; ++i) {
            assert.equal(
                poll.options[i].answer, newPoll.options[i].answer,
                'The new poll doesn\'t have the correct answer');
            assert.equal(
                poll.options[i].votes, newPoll.options[i].votes,
                'The new poll doesn\'t have the correct votes');
            assert.equal(
                poll.options[i].color, newPoll.options[i].color,
                'The new poll doesn\'t have the correct color');
          }
          done();
        });
      });
...
describe('Integration Testing', function() {
  ...
  it('GET /make', function(done) {
    chai.request(server).get('/make').end(function(err, res) {
      assert.equal(res.status, 200, 'response status should be 200');
      done();
    });
  });
  ...
  it('GET /not-a-page', function(done) {
    chai.request(server).get('/not-a-page').end(function(err, res) {
  ...
  describe('e2e testing', function() {
    ...
    const question = 'Can I make a poll?';
    ...
    describe('vote on a poll without authentication', function() {
      ...
      it('should increase the number of votes by 1', function(done) {
        browser.assert.text(
            'span#votes',
            '1' +
                '0'.repeat(poll.options.length - 1));
        done();
      });
    });
            ...
            browser.fill('options[answer1]', options[0]).then(function() {
              browser.fill('options[answer2]', options[1]).then(function() {
                browser.pressButton('Submit', function(err) {
                  // Throws err because zombie has issues with the canvas for
                  // the chart Wrapping done callback in a callback avoids err
                  // being thrown
                  done();
                });
              });
    ...
    describe('vote on poll', function() {
      const otherOption = 'Other answer';

      before(function(done) {
        browser.fill('answer', otherOption).then(function() {
          browser.pressButton('Submit', function(err) {
            // Throws err because zombie has issues with the canvas for the
            // chart Wrapping done callback in a callback avoids err being
            // thrown
            done();
          });
        });
    ...
    describe('delete poll', function() {
      ...
      it('should delete poll in database', function(done) {
        Polls.find({question: question}).exec(function(err, polls) {
          assert.equal(
              polls.length, 0,
              'There shouldn\'t be any polls with the question: "' + question +
                  '" in the database when the poll is deleted.');
          done();
        });
      });
    ...
    describe('fail login with incorrect password and username', function() {
      ...
      it('should alert about incorrect password and username', function(done) {
        browser.assert.text(
            '.alert-danger',
            'Error during login. Username and/or password are incorrect.');
        done();
      });
    ...
    describe('try to signup with an already taken username', function() {
      ...
      it('should show alert about incorrect username', function(done) {
        browser.assert.text(
            '.alert-danger',
            'Error during signup. Please use a different username.');
        done();
      });
